...
void NativeCustomRacer_ReloadRoster(void)
{
    ...
    printf("[CustomRacer] Roster loaded: %d entries\n", s_rosterCount);
}
 ...
 *
 * Layout of model.ctr on disk:
 *     u32 container_size
 *     u8  data[container_size]
 *     u32 num_patches * 4
...
static void ApplyContainerPtrMap(unsigned char *buf, long fileSize)
{
    ...
    unsigned int *patchesHeader = (unsigned int *)(data + dataSize);
    ...
    unsigned int *patches = (unsigned int *)(data + dataSize + 4);
    unsigned int baseAddr = (unsigned int)data;
    ...
    for (unsigned int i = 0; i < numPatches; i++)
    {
        ...
        if (off + 4 > dataSize)
            continue;
        unsigned int *ptr = (unsigned int *)(data + off);
        *ptr = *ptr + baseAddr;
    }
}
...
void *NativeCustomRacer_LoadModel(int characterID)
{
    ...
    if (!buf)
    {
        printf("[CustomRacer] model.ctr not found: %s\n", path);
        return NULL;
    }

    ApplyContainerPtrMap(buf, sz);

    printf("[CustomRacer] model.ctr loaded: %s (%ld bytes)\n", path, sz);
    return buf;
}
...
void NativeCustomRacer_ApplySlot(int characterID)
{
    ...
    if (!buf)
    {
        printf("[CustomRacer] textures.vrm not found: %s\n", path);
        return;
    }

    int blocks = VRM_ApplyBuffer(buf, (int)size);
    free(buf);
