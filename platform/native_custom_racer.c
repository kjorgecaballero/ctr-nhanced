...
static int Page_ParseLine(char *line, PageEntry *out)
{
    ...
    if (*p && *p != '\n' && *p != '\r')
    {
        ...
        if (sscanf(p, "%2x%2x%2x", &r, &g, &b) == 3)
        {
            ...
            out->hasColor = 1;
            p += 6;  /* advance past the 6 hex digits sscanf read */
        }
    }
    ...
    while (fgets(line, sizeof(line), f) && s_pageEntryCount < NATIVE_ROSTER_MAX)
    {
         ...
         * page 0,     slots >= 16: customID = NATIVE_PAGE0_CUSTOM_BASE + (slot-16). */
        {
                ...
                customID <  NATIVE_CUSTOM_ID_BASE + NATIVE_CUSTOM_COUNT)
            {
                ...
                /* Assign a unique voice track base per roster entry.
                 * Track IDs = NATIVE_VOICE_TRACK_BASE + slot*EVENT_COUNT. */
                /* Track base = 314 + roster_index * 8. Uses the raw
                 * roster position (s_pageEntryCount - 1, set just
                 * above), so it matches what build_voice_pipeline.py
                 * computes via enumerate(roster). */
